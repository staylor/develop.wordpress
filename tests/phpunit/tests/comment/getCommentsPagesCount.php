...
class Tests_Comment_GetCommentsPagesCount extends WP_UnitTestCase {
	...
	function test_wp_query_comments_per_page() {
		...
		$this->app['wp']->current_query = new WP_Query( array( 'p' => $post->ID, 'comments_per_page' => 10, 'feed' =>'comments-' ) );
		...
		$this->app['wp']->current_query = new WP_Query( array( 'p' => $post->ID,'comments_per_page' => null, 'feed' =>'comments-' ) );

		$this->assertEquals( 1, get_comment_pages_count() );
		$this->assertEquals( 5, get_comment_pages_count( null, 5 ) );

		$this->app['wp']->current_query->query_vars['comments_per_page'] = null;
	 ...
	 */
	 function test_max_num_comment_pages() {
		...
		$org_max_num_comment_pages = $this->app['wp']->current_query->max_num_comment_pages;

		$this->app['wp']->current_query->max_num_comment_pages = 7;
		...
		$this->app['wp']->current_query->max_num_comment_pages = $org_max_num_comment_pages;
	 }
}
