...
for ($i = 1; $i <= $count; $i++) {
    ...
    foreach ($message as $line) {
        ...
        if ($bodysignal) {
            ...
            if (($content_type == 'multipart/alternative')
                && (false !== strpos($line, 'boundary="')) && ('' === $boundary)
            ) {
                $boundary = trim($line);
            ...
            if (preg_match('/Subject: /i', $line)) {
                ...
                // Captures any text in the subject before
            ...
            /*
             * Set the author using the email address
             * (From or Reply-To, the last used)
            ...
            if (preg_match('/Date: /i', $line)) {
                ...
                // remove parenthesised timezone string if it exists,
                // as this confuses strtotime
    ...
    // Set $post_status based on $author_found and on author's
    ...
    echo "\n<p><strong>" . __('Author:') . '</strong> ' .
        esc_html($post_author) . '</p>';
    echo "\n<p><strong>" . __('Posted title:') . '</strong> ' .
        esc_html($post_title) . '</p>';
