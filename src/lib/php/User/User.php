...
/**
 * Core class used to implement the WP\User\User object.
 *
 * @since 2.0.0
 *
...
class User {
	...
	public function __isset( $key ) {
		if ( 'id' == $key ) {
			_deprecated_argument( 'User->id', '2.1.0',
				sprintf(
	...
	public function _init_caps( $cap_key = '' ) {
		if ( empty( $cap_key ) ) {
			$app = getApp();
			$this->cap_key = $app['db']->get_blog_prefix() . 'capabilities';
		} else {
	...
	public function get_role_caps() {
		...
		if ( is_array( $this->caps ) ) {
			$this->roles = array_filter( array_keys( $this->caps ), [ $app['roles'], 'is_role' ] );
		}
	...
	public function set_role( $role ) {
		if ( 1 == count( $this->roles ) && $role == reset( $this->roles ) ) {
			return;
		}
	...
	public function update_user_level_from_caps() {
		...
		update_user_meta( $this->ID, $app['db']->get_blog_prefix() . 'user_level', $this->user_level );
	}
	...
	public function remove_all_caps() {
		...
		delete_user_meta( $this->ID, $app['db']->get_blog_prefix() . 'user_level' );
		$this->get_role_caps();
	}
