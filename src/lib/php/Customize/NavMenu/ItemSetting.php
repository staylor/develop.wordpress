...
/**
 * Customize API: ItemSetting class
 *
 * @package WordPress
 ...
 *
 * Subclass of \WP\Customize\Setting to represent a nav_menu taxonomy term, and
 * the IDs for the nav_menu_items associated with the nav menu.
 *
 * @since 4.3.0
 */
class ItemSetting extends BaseSetting {
	...
	public function __construct( CustomizeManager $manager, $id, array $args = [] ) {
		...
		if ( isset( $this->value ) ) {
			...
			foreach ( array_diff( array_keys( $this->default ), array_keys( $this->value ) ) as $missing ) {
				throw new CustomizeException( "Supplied nav_menu_item value missing property: $missing" );
			}
		}
	...
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = wp_get_post_terms( $this->post_id, Setting::TAXONOMY, array(
				'fields' => 'ids',
			) );
			if ( ! empty( $menus ) ) {
	 ...
	 *                           entirely. See ItemSetting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
