...
class Manager {
	 ...
	 * @access public
	 *
	 * @param CustomizeManager $manager An instance of the CustomizeManager class.
	 */
	...
	public function enqueue_scripts() {
		...
		$temp_nav_menu_setting      = new Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new ItemSetting( $this->manager, 'nav_menu_item[-1]' );
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => Setting::TYPE,
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && Setting::TYPE === $setting_args['type'] ) {
			$setting_class = Setting::class;
		} elseif ( ! empty( $setting_args['type'] ) && ItemSetting::TYPE === $setting_args['type'] ) {
			$setting_class = ItemSetting::class;
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( Panel::class );
		$this->manager->register_control_type( Control::class );
		$this->manager->register_control_type( NameControl::class );
		$this->manager->register_control_type( AutoAddControl::class );
		$this->manager->register_control_type( ItemControl::class );
		...
		$this->manager->add_panel( new Panel( $this->manager, 'nav_menus', array(
			'title'       => __( 'Menus' ),
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control( new LocationControl( $this->manager, $setting_id, array(
				'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section( new Section( $this->manager, $section_id, array(
			...
			$this->manager->add_setting( new Setting( $this->manager, $nav_menu_setting_id, array(
				'transport' => 'postMessage',
			) ) );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting( new ItemSetting( $this->manager, $menu_item_setting_id, array(
					'value'     => $value,
					'transport' => 'postMessage',
				) ) );

				// Create a control for each menu item.
				$this->manager->add_control( new ItemControl( $this->manager, $menu_item_setting_id, array(
					'label'    => $item->title,
					'section'  => $section_id,
					'priority' => 10 + $i,
				) ) );
	 ...
	 * \WP\Customize\NavMenu\Items::sanitize_nav_menus_created_posts()` to
	 ...
	 * @access public
	 * @see wp_nav_menu()
	 * @see \WP\Customize\Widget\Partial\Refresh::filter_dynamic_sidebar_params()
	 *
