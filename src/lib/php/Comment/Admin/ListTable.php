...
class ListTable extends \WP_List_Table {
	...
	public function prepare_items() {
		$comment_status = $this->_request->get( 'comment_status', 'all' );
		...
		$this->app->set( 'comment_type', $comment_type );

		$search = $this->_request->get( 's', '' );
		$this->app->set( 'search', $search );
		...
		$number = $this->_request->getInt(
			'number',
			// Grab a few extra
			$comments_per_page + min( 8, $comments_per_page )
		);

		$page = $this->get_pagenum();

		$start = $this->_request->getInt(
			'start',
			( $page - 1 ) * $comments_per_page
		);

		if ( $doing_ajax && $this->_request->getInt( 'offset' ) ) {
			$start += $this->_request->getInt( 'offset' );
		}
	...
	public function no_items() {
		if ( 'moderated' === $this->app->get( 'comment_status' ) ) {
	...
	protected function get_views() {
		$status_links = [];
		$post_id = $this->app->get( 'post_id' );
		$num_comments = 0 === $post_id ? wp_count_comments() : wp_count_comments( $post_id );

		$comment_type = $this->app->get( 'comment_type' );
...
<?php
	...
	public function single_row( $comment ) {
		...
		$this->app->set( 'comment_status', $comment_status );
		$the_comment_class = join( ' ', get_comment_class( $comment_status, $comment, $comment->comment_post_ID ) );
