...
function wp_new_comment( $commentdata, $avoid_die = false ) {
	...
	if ( ! isset( $commentdata['comment_author_IP'] ) ) {
		$commentdata['comment_author_IP'] = $app['request.remote_addr'];
	}
	$commentdata['comment_author_IP'] = preg_replace( '/[^0-9a-fA-F:., ]/', '', $commentdata['comment_author_IP'] );

	if ( ! isset( $commentdata['comment_agent'] ) ) {
		$commentdata['comment_agent'] = $app['request.useragent'] ?? '';
	}
	...
	if ( $to_ping ) {
		foreach ( (array) $to_ping as $tb_ping ) {
			...
			if ( !in_array($tb_ping, $pinged) ) {
				...
				$sql = 'UPDATE ' . $wpdb->posts . ' SET to_ping = TRIM(REPLACE(to_ping, %s, "")) WHERE ID = %d';
				$wpdb->query( $wpdb->prepare( $sql, $tb_ping, $post->ID ) );
			}
		}
	}
}
