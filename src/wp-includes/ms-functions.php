...
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	if ( !is_subdomain_install() && $wpdb->get_var( $wpdb->prepare( "SELECT post_name FROM " . $wpdb->get_blog_prefix( get_network()->blog_id ) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname ) ) ) {
...
function install_blog( int $blog_id, $blog_title = '' ) {
	...
	if ( ! is_subdomain_install() ) {
		...
		if ( 'https' === parse_url( get_home_url( get_network()->blog_id ), PHP_URL_SCHEME ) ) {
	...
	// remove all perms
	delete_metadata( 'user', 0, $app['db.table_prefix'] . 'user_level',   null, true ); // delete all
	delete_metadata( 'user', 0, $app['db.table_prefix'] . 'capabilities', null, true ); // delete all
}
